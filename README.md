...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/himanshub334/market-research-platform)
...
No environment variables required for the base deployment (uses in-memory store).

To connect a real PostgreSQL database, add:

```env
DATABASE_URL=postgresql://user:password@host:5432/market_db
```

Then swap `dataStore.ts` to use `pg` or `@vercel/postgres`.
...
## Extending to Real PostgreSQL

The `dataStore.ts` singleton is a drop-in replacement layer. To connect a real DB:

1. Install `@vercel/postgres`: `npm install @vercel/postgres`
2. Add `DATABASE_URL` to `.env.local`
3. Replace `marketStore.getOHLCV()` calls with SQL queries like:
...
-- BRIN index (already defined in schema)
CREATE INDEX idx_price_brin ON price_data USING BRIN (timestamp);

-- Materialised view for OHLCV aggregates
REFRESH MATERIALIZED VIEW CONCURRENTLY ohlcv_daily;
```
